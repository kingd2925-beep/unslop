...
import { swapColor, swapFont, readThemeCss, applyBrandToDocument } from './dom-theme.js';
...
export function createApp(els) {
  ...
  function applyBrand(brand) {
    const result = applyBrandToDocument(doc, brand);
    if (!result.changed) {
      toast(result.rolesFound ? 'This page already uses this look.' : 'Could not tell which colours are background, text and accent on this page. Change them from the Colours list instead.');
      return;
    }
    commit();
    refreshPanels();
    toast(`Applied "${brand.name}": ${result.colors} colour${result.colors === 1 ? '' : 's'} and ${result.fonts} font${result.fonts === 1 ? '' : 's'}. Undo if you don't like it.`, 'success');
  }
